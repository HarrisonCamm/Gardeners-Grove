...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const imagePickers = document.querySelectorAll('.image_picker');
    ...
    imagePickers.forEach(label => {
        label.addEventListener('click', function(event) {
            const labelID = label.getAttribute('image-button-id');
            if (labelID != null) {
                id = labelID;
                images.forEach(tmpImage => { //Should only be one image
                    if (tmpImage.getAttribute('data-plant-id') === labelID) {
                        image = tmpImage;
                        console.log("Image for plant id: " + labelID)
                    } else if (tmpImage.getAttribute('data-user-id') === labelID) {
                        image = tmpImage;
                        console.log("Image for user id: " + labelID)
                    }
                });
            }
        });
    ...
    // submitButton != null && submitButton.addEventListener('click', async function(event) {
    //     await submitAction(file, id);
    // });
});
